...
#   RELAY_HOSTNAME          (relays.to)          public hostname of the server
...
RELAY_HOSTNAME="${RELAY_HOSTNAME:-relays.to}"
...
# for a bad pairing is opaque. Check it here instead.
log "Checking that $SERVER_TYPE is offered in $SERVER_LOCATION"
if ! SERVER_TYPE_JSON="$(hcloud server-type describe "$SERVER_TYPE" -o json 2>/dev/null)"; then
    fail "hcloud does not know the server type \"$SERVER_TYPE\""
fi
SERVER_TYPE_LOCATIONS="$(printf '%s' "$SERVER_TYPE_JSON" | jq -r '[.prices[].location] | join(", ")')"
case ", $SERVER_TYPE_LOCATIONS, " in
    *", $SERVER_LOCATION,"*) ;;
    *)
        fail "$SERVER_TYPE is not offered in $SERVER_LOCATION. Available locations: $SERVER_TYPE_LOCATIONS"
        ;;
esac
...
for index in "${!SMTP_FLOATING_IP_NAMES[@]}"; do
    hcloud floating-ip set-rdns \
        --ip "${SMTP_POOL[$index]}" \
        --hostname "sender-$((index + 1)).mail.${RELAY_HOSTNAME}" \
        "${SMTP_FLOATING_IP_NAMES[$index]}"
done

log "DNS zone $RELAY_HOSTNAME"
if hcloud zone describe "$RELAY_HOSTNAME" >/dev/null 2>&1; then
    log "Zone $RELAY_HOSTNAME already exists"
else
    hcloud zone create --name "$RELAY_HOSTNAME"
    for name in "@" "*" ns1 ns2 mx1 mx2 smtp; do
        hcloud zone rrset set-records --record "$SERVER_IP" "$RELAY_HOSTNAME" "$name" A
    done
fi

# Each egress address needs a forward record for its PTR, and this set changes
# whenever the pool does, so it is rewritten on every run.
for index in "${!SMTP_POOL[@]}"; do
    hcloud zone rrset set-records \
        --record "${SMTP_POOL[$index]}" \
        "$RELAY_HOSTNAME" "sender-$((index + 1)).mail" A
done

ZONE_JSON="$(hcloud zone describe "$RELAY_HOSTNAME" -o json)"
ZONE_NAMESERVERS="$(printf '%s' "$ZONE_JSON" | jq -r '[.authoritative_nameservers.assigned[]?] | join(", ")')"
ZONE_DELEGATION="$(printf '%s' "$ZONE_JSON" | jq -r '.authoritative_nameservers.delegation_status // "unknown"')"
...
Provisioning complete.
  ...
  DNS zone           $RELAY_HOSTNAME (delegation: $ZONE_DELEGATION)

Next steps:

  1. Delegate $RELAY_HOSTNAME at your registrar to:

       $ZONE_NAMESERVERS

     Then confirm the delegation with:

       hcloud zone describe $RELAY_HOSTNAME -o json | jq .authoritative_nameservers
