...
class TestBuildChartData:
    def test_build_chart_data__returns_series_for_all_choices(self):
        result = build_chart_data([], list(Status), COLORS, date(2026, 1, 1), "status")
    ...
    def test_build_chart_data__uses_muted_color_for_unknown_choice(self):
        class Extra(TextChoices):
            UNKNOWN = "unknown", "Unknown"

        result = build_chart_data([], [Extra.UNKNOWN], {}, date(2026, 1, 1), "status")
        assert result["series"][0]["color"] == "var(--color-muted-foreground)"
