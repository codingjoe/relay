...
| `postmaster` (+extensions) | Human mail to postmaster        | Store, then forward a summary to your team   |
...
with a plain-text alternative. A worker sends it, so it is written in the
platform's default language.
