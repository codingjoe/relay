...
- **Signing covers the message as stored.** relay signs with the private
  ...
  signature relay applied in its own card, verifies each signature
  against the DKIM keys relay's nameserver publishes, and shows the
  verdict next to the signing domain, selector, and algorithm. The raw
  tag list is one click away.
