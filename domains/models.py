...
class Domain(TimeStamped):
    ...
    def compute_reputation(self):
        """Return bounce and complaint rates for this domain over the rolling window.
        ...
        total_sent = outgoing.count()

        transmissions = Transmission.objects.filter(
            message__domain=self,
    ...
    def check_reputation(self):
        ...
        exceeds = (
            ...
            or stats["complaint_rate"]
            > settings.RELAY_REPUTATION_COMPLAINT_RATE_THRESHOLD
        )

        if exceeds and not self.reputation_hold:
            self.reputation_hold = True
            self.reputation_hold_at = timezone.now()
            self.save(
                update_fields=["reputation_hold", "reputation_hold_at", "modified_at"]
            )
        elif not exceeds and self.reputation_hold:
            self.reputation_hold = False
            self.reputation_hold_at = None
            self.save(
                update_fields=["reputation_hold", "reputation_hold_at", "modified_at"]
            )
