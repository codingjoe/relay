...
Generated by 'django-admin startproject' using Django 6.1rc1.
...
# Django 6.1 task framework
# In production, Threadmill provides a durable Redis-backed task queue with
# a worker process (manage.py threadmill worker). In tests and development,
# ImmediateRetryBackend runs tasks synchronously so no worker process is
# needed. It accepts Threadmill's retry parameter but never calls the retry
# callback — tasks that raise are simply marked FAILED.
