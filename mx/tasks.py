...
def schedule_retry(message_id, webhook, attempt):
    """Schedule the next retry attempt, or give up past the schedule."""
...
def deliver_with_retries(message, webhook, attempt, is_final_attempt):
    """Attempt delivery; on failure, schedule the next retry per the spec."""
...
@dataclass
class WebhookEvent:
    """A flat, JSON-serialisable webhook event payload.

    ``body_url`` is generated by the storage backend (presigned S3 URL in
    production). The raw body is never included inline.
    """
...
class WebhookJSONEncoder(DjangoJSONEncoder):
    """JSON encoder that natively serialises ``WebhookEvent`` instances."""
...
def deliver_to_webhook(message, webhook, is_test=False):
    """POST a Standard Webhooks compliant signed payload. Return (ok, status_code)."""
